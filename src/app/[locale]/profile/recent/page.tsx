...
export default function RecentPlaysPage() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push(`/${locale}/signin?callbackUrl=${encodeURIComponent(`/${locale}/profile/recent`)}`);
      return;
    }
    ...
    const fetchRecentPlays = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/user/recent?limit=50`, {
          headers: {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch recent plays: ${response.statusText}`);
        }
        ...
        setError("Failed to load your recent plays. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="container mx-auto py-10 px-6 flex items-center justify-center min-h-[calc(100vh-200px)]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground">Loading your recent plays...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto py-10 px-6 flex items-center justify-center min-h-[calc(100vh-200px)]">
        <Card className="w-full max-w-md border-red-400/20 bg-red-500/5">
          <CardHeader>
            <CardTitle className="text-red-500">Error Loading Recent Plays</CardTitle>
          </CardHeader>
          <CardContent>
